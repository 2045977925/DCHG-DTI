...
if __name__ == '__main__':
    ...
    # sdf提取信息
    sdf_file_paths = [
        'graphdata/drugs/3D_structures.sdf',  # SDF 文件路径
        'graphdata/drugs/approved_drugs.sdf'  # SDF 文件路径
    ]
    extract_sdf_info(sdf_file_paths)
    # 合并
    smiles_file_path = 'graphdata/drugs/drug_atoms_edges_in_3D_structures.csv'
    atoms_edges_file_path = 'graphdata/drugs/drug_atoms_edges_in_approved_drugs.csv'
    output_file_path = 'graphdata/drugs/merged_drug_atoms_edges.csv'  # 输出合并后的文件路径
    # 调用函数合并CSV文件
    merge_csv_files(smiles_file_path, atoms_edges_file_path, output_file_path)
    # 提取
    smiles_file_path = 'datainput/drug_smiles.csv'  # drug_smiles.csv的路径
    merged_file_path = 'graphdata/drugs/merged_drug_atoms_edges.csv'  # merged_drug_atoms_edges.csv的路径
    output_file_path = 'graphdata/drugs/drug_graph_data.csv'  # 输出提取后的文件路径
    extract_rows_based_on_smiles(smiles_file_path, merged_file_path, output_file_path)
    # 构建药物分子图
    input_file_path = 'graphdata/drugs/drug_graph_data.csv'  # 输入文件路径
    output_file_path = 'graphdata/drugs/drug_graph.csv'  # 输出文件路径
    # 调用函数构建药物分子图
    construct_drug_graph(input_file_path, output_file_path)

    # 通道B构建蛋白质残基接触图
    # 计算残基接触图
    calculate_protein_contact_maps('datainput/PDB_3D_info', 'datainput/Protein_Contact_Maps', ca_distance_threshold=8.0,
                                   ligand_distance_threshold=5.0)

    # 残基接触图转换为邻接矩阵
    create_adjacency_matrices('datainput/Protein_Contact_Maps', 'datainput/Uniprot_PDB.csv', 'graphdata/proteins/protein_graph.csv')
